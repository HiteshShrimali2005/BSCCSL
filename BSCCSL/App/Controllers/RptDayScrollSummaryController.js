angular.module("BSCCL").controller('RptDayScrollSummaryController', function ($scope, AppService, $state, $cookies, $filter, $location, $rootScope) {
    ...
    //$rootScope.ChangeBranch = function (brid) {
    //    var branchid = $("#ddlBranch option:contains(" + brid + ")").val();
    //    $cookies.put('Branch', branchid.replace('string:', ''))
    //    $scope.BranchHO = $filter('filter')($scope.BranchList, { BranchId: $cookies.get('Branch') })[0].IsHO;
    //    //  $('#tblAccountsCRDR').dataTable().fnDestroy();
    //    GetAccountsCreditDebit();
    //}
    ...
    function GetAccountsCreditDebit() {
        ...
        var getdata = AppService.GetDataByQrysr("Report", "RptSummaryCRDR", $scope.UserBranch.BranchId, ($scope.FinYear ? $scope.FinYear : ""), startDate, endDate)
    ...
    function GetSummaryCreditDebit(branchid) {
        ...
        var getdata = AppService.GetDataByQrysr("Report", "RptSummaryCRDR", branchid, ($scope.FinYear ? $scope.FinYear : ""), startDate, endDate)
        getdata.then(function (p1) {
            if (p1.data) {
                $scope.AccountsCRDR = p1.data.rptlist;
                if ($scope.RptAccountsCRDRViewModel == undefined)
                ...
                //GetAccountsHeadListParentHead()
                //GetAccountsHeadList()
              //  $("#ddlBranch value:" + branchid + "").attr('selected', 'selected');
    ...
    $scope.BranchChangeCRDR = function (bId) {

        var branchid = $("#ddlBranch option:contains(" + bId + ")").val();
        //$cookies.put('Branch', branchid.replace('string:', ''))
        //$scope.BranchHO = $filter('filter')($scope.BranchList, { BranchId: $cookies.get('Branch') })[0].IsHO;
        //GetSummaryCreditDebit(branchid.replace('string:', ''));
        $scope.UserBranch.BranchId = branchid.replace('string:', '');
        $("#ddlBranch option:contains(" + bId + ")").attr('selected', 'selected');
