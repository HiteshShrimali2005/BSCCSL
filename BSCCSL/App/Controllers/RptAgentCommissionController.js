angular.module("BSCCL").controller('RptAgentCommissionController', function ($scope, AppService, $state, $cookies, $filter, $location, $rootScope) {
        ...
        AgentCommissionReport = $('#tblAgentCommission').DataTable({
            ...
            "ajax": function (data, callback, settings) {
                ...
                $.ajax({
                    "dataType": 'json',
                    "type": "POST",
                    "url": urlpath + "/Report/GetAllAgentCommissionList",
                    "data": data,
    ...
    $('#tblAgentCommission tbody').on('click', 'td.details-control', function () {
        ...
        if (row.child.isShown()) {
            ...
            NewObj.ProductName = $("#txtProductName").val();


            AppService.SaveData("Report", "GetCommissionData", NewObj).then(function (p1) {
                if (p1.data) {
                    var ComData = p1.data;
                    var detail = '';
                    if (ComData.length > 0)
                        ...
                        detail += '<tr>' +
                                        ...
                                        '<td>' + (ComData[i].ProductName === null ? '' : ComData[i].ProductName) + '</td>' +
                                         ...
                                         '<td class="text-right">' + ($filter('currency')(ComData[i].Amount, '₹ ', 2)) + '</td>' +
                                          '<td class="text-center">' + ((ComData[i].agentCommission * 100) / ComData[i].Amount) + '</td>' +
