angular.module("BSCCL").controller('TransactionController', function ($scope, AppService, $state, $cookies, $filter, $location, $rootScope) {
    ...
    function GetHolderDetail() {
        ...
        getholder.then(function (p1) {
            if (p1.data.AccountDetail != null) {
                ...
                if ($scope.HolderData.ProductName == 'Akshaya Tritiya') {
                    var customerRDList = AppService.GetDetailsById("CustomerProduct", "GetCustomerMappedRDAccount", $scope.HolderData.CustomerProductId);
                    customerRDList.then(function (p2) {
                        if (p2.data != null) {
                            $scope.RDList = p2.data;
                        }
                ...
                },
                {
                    ...
                    "mRender": function (data, type, full) {
                        if (full.TransactionType !== 1 && full.TransactionType !== 0 && full.Status == 1) {
                            ...
                            if (LastTransId == data) {
                                if ($scope.Role != "1") {
                                    if (full.Status == 0 && $scope.HolderData.ProductType == 1 && full.TransactionTime.split('T')[0] == CurrentDate.split('T')[0]) {
                                    ...
                                    if (full.Status == 0 && $scope.HolderData.ProductType == 1) {
                                        return '<button class="btn btn-danger btn-xs btnDeleteTransaction" Id="' + data + '" title="DeleteTransaction"><span class="glyphicon glyphicon-trash"></span> Delete </button>'
                                    }
                                    else {
                                        return '';
