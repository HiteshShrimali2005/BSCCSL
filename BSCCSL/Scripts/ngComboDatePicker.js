...
.directive('ngComboDatePicker', function() {
    ...
    return {
        ...
        template: function(element, attrs) {
            ...
            // Generate HTML code.
            var html =
                '<select style="display:none;" ng-model="date" '+strAttrs[0]+' ng-options="date.value as date.name disable when date.disabled for date in dates"></select>' +
                '<select style="display:none;" ng-model="month" ' + strAttrs[1] + ' ng-options="month.value as month.name disable when month.disabled for month in months"></select>' +
                '<select id="ddlSearchYear" ng-model="year" ' + strAttrs[2] + ' ng-options="year.value as year.name disable when year.disabled for year in years"><option value="">Select</option></select>'
            ;
