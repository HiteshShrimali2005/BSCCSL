...
{
    public class GeneralLedgerDetailsService
    {
        ...
        public object GetGeneralLedgerList(DateTime FromDate, DateTime ToDate, Guid BranchId, Guid AccountId,User user)
        {
            using (var db = new BSCCSLEntity())
            {
                ...
                SqlConnection sql = new SqlConnection(connectionstring);
                SqlCommand cmdTimesheet = new SqlCommand("GetGeneralLedgerDetails", sql);
                cmdTimesheet.CommandType = CommandType.StoredProcedure;
                //Create a parameter using the new SQL DB type viz. Structured to pass as table value parameter
                ...
                SqlParameter paramAccountId = cmdTimesheet.Parameters.Add("@AccountId", SqlDbType.UniqueIdentifier);
